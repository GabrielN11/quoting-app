...
export default function ShareUserList({navigation, route}) {
    ...
    async function getUsers(){
        setLoading(true)
        try{
            const json = await fetch(`${API_URL}/users-by-share/${id}?page=${page}`)
            if(json.status === 200){
  ...
  return (
    <ScrollView style={{backgroundColor: colors.BACKGROUND, height: '100%'}}>
      ...
      <ProfileText style={{alignSelf: 'center'}}>People who liked this</ProfileText>
      {users.map(user => (
